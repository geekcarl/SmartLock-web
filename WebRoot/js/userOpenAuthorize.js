...
function fastsearch() {
	...
	if (log_name != '' || grant_user != '' || user_name != '' || operator_name != '')
	{
		...
		postData.operator_username = user_name;
		postData.operator_name = operator_name;
		$("#manage-grant-grid-table").jqGrid('setGridParam',{ 
          url:'lockandkey.do?method=listUserOpenGrantLogPageByAjax',//你的搜索程序地址 
	...
	$.ajax( {
		...
		success : function(data) {
			...
			if (data.resultMark == 1)
			{
				...
				$('#view_card_no').html(data.object.card_no);
				$('#view_card_rfid').html(data.object.card_rfid);
				$('#view_remarks').html(data.object.remarks);
				if (data.object.gds.length > 0) $('#view_date').html(new Date(data.object.gds[0].begin_time).Format('yyyy-MM-dd hh:mm:ss') + '至' + new Date(data.object.gds[0].end_time).Format('yyyy-MM-dd hh:mm:ss'));
				...
				var dialog = $( "#dialog-view" ).removeClass('hide').dialog({
					...
					width: 900,
					height: 600,
					title: "<div class='widget-header widget-header-small'><h4 class='smaller'>人员卡授权(" + data.object.log_name + ")</h4></div>",
					title_html: true,
					buttons: [ 
						{
							text: "确定",
...
jQuery(function($) {
	...
	jQuery(manage_grant_grid_selector).jqGrid({
		// direction: "rtl",
		url: 'lockandkey.do?method=listUserOpenGrantLogPageByAjax',
		datatype: 'json',
		mtype: 'post',
		prmNames: {sort: 'sort', order: 'order'},
		height: 'auto',
		loadtext: '加载中...',
		colNames:['ID','授权任务名称', '授权人', '授权时间', '用户名', '姓名', '授权光交箱信息', '权限开始时间', '权限结束时间'],
		colModel:[
			...
			{name:'operators.user_name',index:'operators'},
			{name:'operators_name',index:'operators_name'},
			...
			{name:'gds',index:'begin_time', sortable:false, formatter:function(cellvalue, options, rowObject){
					...
					if (rowObject.gds != null && rowObject.gds.length > 0)
					{
						...
						{
							temp = new Date(rowObject.gds[0].begin_time).Format("yyyy-MM-dd hh:mm:ss");
						}
					}
					else
					{
			...
			{name:'gds',index:'end_time', sortable:false, formatter:function(cellvalue, options, rowObject){
					...
					if (rowObject.gds != null && rowObject.gds.length > 0)
					{
						...
						{
							temp = new Date(rowObject.gds[0].end_time).Format("yyyy-MM-dd hh:mm:ss");
						}
					}
					else
					{
	...
	jQuery(manage_grant_grid_selector).jqGrid('navGrid', manage_grant_pager_selector,
		{ 	// navbar options
			edit: $('#opendoorAuthorize_update').val() == 'true' ? true : false,
			editfunc: function(){
					...
					$.ajax( {
						...
						success : function(data) {
							...
							if (data.resultMark == 1)
							{
								...
								$('#u_grant_begin_time').val(_data.object.gds.length > 0 && _data.object.gds[0].begin_time != null ? new Date(_data.object.gds[0].begin_time).Format("yyyy-MM-dd hh:mm:ss") : '');
								$('#u_grant_end_time').val(_data.object.gds.length > 0 && _data.object.gds[0].begin_time != null ? new Date(_data.object.gds[0].end_time).Format("yyyy-MM-dd hh:mm:ss") : '');
								...
								var dialog = $("#dialog-update-grant").removeClass('hide').dialog({
									...
									width: 1000,
									height: 800,
									title: "<div class='widget-header widget-header-small'><h4 class='smaller'>修改用户开门授权</h4></div>",
									title_html: true,
									buttons: [ 
										{
											text: "确定",
											"class" : "btn btn-primary btn-xs",
											click: function() {
												if ($('#form-update-grant').valid() &&  g_obj.operators_sels.length > 0 && g_obj.operators_sels.length > 0)
												{
													var grant = '';
												...
												//	grant += '&key_id=' + g_obj.keys_sels[1].id;
													...
													$.ajax( {
														type : "post",
														url : "lockandkey.do?method=updateUserOpenGrantLog",
														data: grant,
														success : function(data) {
															isOvertime(data.resultMark);
															if (data.resultMark == 1)
															{
																$('#manage_grant_warn').show();
																$('#manage_grant_warn_label').html('修改用户开门授权成功！');
																mySetTimeOut('manage_grant_warn', 4000);
															...
															{
																$('#manage_grant_warn').show();
																$('#manage_grant_warn_label').html('修改用户开门授权失败，请重试！');
																mySetTimeOut('manage_grant_warn', 4000);
															}
						...
						error : function() {
							$('#manage_grant_warn').show();
							$('#manage_grant_warn_label').html('获取用户开门权限出错，请重试！');
							mySetTimeOut('manage_grant_warn', 4000);
						}
					});
				}
			...
			add: $('#opendoorAuthorize_add').val() == 'true' ? true : false,
			addfunc: function(){
				g_obj.keys_sels = [];
				g_obj.boxes_sels = [];
				g_obj.operators_sels = [];
				...
				var dialog = $( "#dialog-add-grant" ).removeClass('hide').dialog({
					...
					width: 1000,
					height: 800,
					title: "<div class='widget-header widget-header-small'><h4 class='smaller'>添加用户开门授权</h4></div>",
					title_html: true,
					buttons: [ 
						{
							text: "添加",
							"class" : "btn btn-primary btn-xs",
							click: function() {
								if ($('#form-add-grant').valid()  && g_obj.operators_sels.length > 0 && g_obj.operators_sels.length > 0)
								{
									var grant = '';
									...
									$.ajax( {
										type : "post",
										url : "lockandkey.do?method=addUserOpenGrantLog",
										data: grant,
										success : function(data) {
											isOvertime(data.resultMark);
											if (data.resultMark == 1)
											{
												$('#manage_grant_warn').show();
												$('#manage_grant_warn_label').html('添加用户开门授权成功！');
												mySetTimeOut('manage_grant_warn', 4000);
											...
											{
												$('#manage_grant_warn').show();
												$('#manage_grant_warn_label').html('添加用户开门授权失败，请重试！');
												mySetTimeOut('manage_grant_warn', 4000);
											}
			...
			del: $('#opendoorAuthorize_delete').val() == 'true' ? true : false,
			delfunc : function () {
				var array = $('#manage-grant-grid-table').jqGrid('getGridParam','selarrrow');
				if (array.length <= 0)
				{
					...
					if (confirm("删除是不可恢复的，你确认要删除吗？"))
					{
						...
						$.ajax( {
							...
							success : function(data) {
								...
								{
									$('#manage_grant_warn').show();
									$('#manage_grant_warn_label').html('删除用户开门授权成功！');
									mySetTimeOut('manage_grant_warn', 4000);
									$('#manage-grant-grid-table').trigger("reloadGrid");
								}
								else
								{
									$('#manage_grant_warn').show();
									$('#manage_grant_warn_label').html('删除用户开门授权失败，请重试！');
									mySetTimeOut('manage_grant_warn', 4000);
								}
							},
							error : function() {
			...
			view: $('#workcardAuthorize_view').val() == 'true' ? true : false,
			viewfunc: function () {
				var array = $('#manage-grant-grid-table').jqGrid('getGridParam','selarrrow');
				if (array.length <= 0)
				{
