		...
		if (confirm('删除了无法恢复，确定要删除该记录吗？'))
		{
			...
			$.ajax( {
				...
				success : function(data) {
					...
					{
						$('#manage_grant_warn').show();
						$('#manage_grant_warn_label').html('删除角色记录成功！');
						mySetTimeOut('manage_grant_warn', 4000);
					...
					{
						$('#manage_grant_warn').show();
						$('#manage_grant_warn_label').html('删除角色记录失败，请重试！');
						mySetTimeOut('manage_grant_warn', 4000);
					}
				},
				error : function() {
...
jQuery(function($) {
	...
	jQuery(manage_grant_grid_selector).jqGrid({
		// direction: "rtl",
		url: 'user.do?method=roleListByPager',
		datatype: 'json',
		mtype: 'post',
	...
	jQuery(manage_grant_grid_selector).jqGrid('navGrid', manage_grant_pager_selector,
		{ 	// navbar options
			edit: $('#role_update').val() == 'true' ? true : false,
			editfunc : function () {
				var array = $('#manage-grant-grid-table').jqGrid('getGridParam','selarrrow');
				if (array.length <= 0)
				{
				...
				{
					var rowdata = $('#manage-grant-grid-table').jqGrid('getRowData', array[0]);
					$('#u_id').val(rowdata.id);
					$('#u_name').val(rowdata.name);
					$('#u_remark').val(rowdata.remark);
					$.ajax( {
						type : "post",
						url : "user.do?method=findRole",
						data : "id=" + rowdata.id,
						success : function(data) {
							isOvertime(data.resultMark);
							if (data.resultMark == 1)
							{
								...
								$.ajax( {
									type : "post",
									url : "user.do?method=findAllPrivileges",
									success : function(data) {
										isOvertime(data.resultMark);
										if (data.resultMark == 1)
										{
											$('#u_tree_div').html('<div id="u_tree" class="ztree"></div>');
											var setting = {
												view: {
													selectedMulti: true,
													showIcon: false
												...
												callback: {
													beforeCheck: function (treeId, treeNode) {
														var zTree = $.fn.zTree.getZTreeObj("u_tree");
														g_obj.u_sels = zTree.getCheckedNodes(true);
													},
											...
											g_obj.u_tree = $.fn.zTree.init($("#u_tree"), setting, data.tree);
											for (var i = 0; i < _data.object.rps.length; i ++)
											{
												var node = g_obj.u_tree.getNodeByParam('id', _data.object.rps[i].privilege.id);
												g_obj.u_tree.checkNode(node, true, false, true);
											}
										}
										else
										{
											$('#manage_grant_warn').show();
											$('#manage_grant_warn_label').html('加载权限失败，请重试！');
											mySetTimeOut('manage_grant_warn', 4000);
										}
									},
									error : function() {
							...
							{
								$('#manage_grant_warn').show();
								$('#manage_grant_warn_label').html('加载角色失败，请重试！');
								mySetTimeOut('manage_grant_warn', 4000);
							}
						},
						error : function() {
					...
					var dialog = $( "#dialog-u-role" ).removeClass('hide').dialog({
						modal: true,
						width: 1000,
						height: 500,
						title: "<div class='widget-header widget-header-small'><h4 class='smaller'>修改角色</h4></div>",
						title_html: true,
						buttons: [ 
							{
								text: "确定",
								...
								click: function() {
									...
									else
									{
										...
										data += '&remark=' + $('#u_remark').val();
										for (var i = 0; i < g_obj.u_sels.length; i ++)
										{
											data += '&rps[' + i + '].role.id=' + $('#u_id').val();
											data += '&rps[' + i + '].privilege.id=' + g_obj.u_sels[i].id; 
										}
										$.ajax( {
											type : "post",
											url : "user.do?method=updateRole",
											data: data,
											success : function(data) {
												isOvertime(data.resultMark);
												if (data.resultMark == 1)
												{
													$('#manage_grant_warn').show();
													$('#manage_grant_warn_label').html('修改角色成功！');
													mySetTimeOut('manage_grant_warn', 4000);
													$('#manage-grant-grid-table').trigger("reloadGrid");
													if ($('#user_id').val() == $('#u_id').val())
													{
														alert('重新登录才会生效，点击返回登录页面');
												...
												{
													$('#manage_grant_warn').show();
													$('#manage_grant_warn_label').html('修改角色失败，请重试！');
													mySetTimeOut('manage_grant_warn', 4000);
												}
												$( "#dialog-u-role" ).dialog( "close" ); 
											},
											error : function() {
												$('#manage_grant_warn').show();
												$('#manage_grant_warn_label').html('修改角色失败，请重试！');
			...
			add: $('#role_add').val() == 'true' ? true : false,
			addfunc : function () {
				$.ajax( {
					type : "post",
					url : "user.do?method=findAllPrivileges",
					success : function(data) {
						...
						if (data.resultMark == 1)
						{
							$('#a_tree_div').html('<div id="a_tree" class="ztree"></div>');
							var setting = {
								view: {
									selectedMulti: true,
									showIcon: false
								...
								callback: {
									beforeCheck: function (treeId, treeNode) {
										var zTree = $.fn.zTree.getZTreeObj("a_tree");
										g_obj.a_sels = zTree.getCheckedNodes(true);
									},
									onCheck: function (e, treeId, treeNode) {
										var zTree = $.fn.zTree.getZTreeObj("a_tree");
										g_obj.a_sels = zTree.getCheckedNodes(true);
							...
							g_obj.a_tree = $.fn.zTree.init($("#a_tree"), setting, data.tree);
							var dialog = $( "#dialog-a-role" ).removeClass('hide').dialog({
								modal: true,
								width: 1000,
								height: 500,
								...
								buttons: [ 
									{
										...
										click: function() {
											...
											{
												var data = '';
												data += 'name=' + $('#a_name').val();
												data += '&remark=' + $('#a_remark').val();
												for (var i = 0; i < g_obj.a_sels.length; i ++)
												{
													data += '&rps[' + i + '].privilege.id=' + g_obj.a_sels[i].id; 
												}
												$.ajax( {
													type : "post",
													url : "user.do?method=addRole",
													data: data,
													success : function(data) {
														isOvertime(data.resultMark);
														if (data.resultMark == 1)
														{
															$('#manage_grant_warn').show();
															$('#manage_grant_warn_label').html('添加角色成功！');
															mySetTimeOut('manage_grant_warn', 4000);
															$('#manage-grant-grid-table').trigger("reloadGrid");
														}
														else
														{
															$('#manage_grant_warn').show();
															$('#manage_grant_warn_label').html('添加角色失败，请重试！');
															mySetTimeOut('manage_grant_warn', 4000);
														}
														$( "#dialog-a-role" ).dialog( "close" ); 
													},
													error : function() {
														$('#manage_grant_warn').show();
														$('#manage_grant_warn_label').html('添加角色失败，请重试！');
			...
			del: $('#role_delete').val() == 'true' ? true : false,
			delfunc : function () {
				delete_alarm();
			},
