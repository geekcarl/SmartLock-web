...
function delete_alarm()
{
	...
	else
	{
		if (confirm('删除了无法恢复，确定要删除该记录吗？'))
		{
			...
			$.ajax( {
				type : "post",
				url : "user.do?method=deleteMessage",
				data: "ids=" + alarms,
				success : function(data) {
					...
					{
						$('#manage_grant_warn').show();
						$('#manage_grant_warn_label').html('删除短信记录成功！');
						mySetTimeOut('manage_grant_warn', 4000);
					...
					{
						$('#manage_grant_warn').show();
						$('#manage_grant_warn_label').html('删除短信记录失败，请重试！');
						mySetTimeOut('manage_grant_warn', 4000);
					}
				},
				error : function() {
...
function fastsearch() {
	...
	var create_date = $('#f_s_create_date').val();
	if (full_name != '' || create_date != '')
	{
		...
		if (create_date != '')
		{
			postData.create_date = create_date + ' 12:12:12';
		}
		$("#manage-grant-grid-table").jqGrid('setGridParam',{ 
            url:'user.do?method=messageListByPager',//你的搜索程序地址 
...
jQuery(function($) {
	...
	jQuery(manage_grant_grid_selector).jqGrid({
		// direction: "rtl",
		url: 'user.do?method=messageListByPager',
		datatype: 'json',
		mtype: 'post',
		rownumbers:true,
		...
		height: 'auto',
		loadtext: '加载中...',
		colNames:['id', '接收人', '部门', '联系方式', '短信内容', '告警时间', '发送时间'],
		colModel:[
		    {name:'id',index:'id',hidden:true},
			{name:'full_name',index:'full_name',width:60,sortable:false},
			{name:'name',index:'name',width:70,sortable:false},
			{name:'recipient',index:'recipient',width:80,sortable:false},
			{name:'text',index:'text',width:250,sortable:false},
			{name:'create_date',index:'create_date', width:100, formatter:function(cellvalue, options, rowObject){
					return rowObject.create_date == null ? "" : new Date(rowObject.create_date).Format("yyyy-MM-dd hh:mm:ss");
				}
			},
			{name:'sent_date',index:'sent_date', width:100, formatter:function(cellvalue, options, rowObject){
					return rowObject.sent_date == null ? "" : new Date(rowObject.sent_date).Format("yyyy-MM-dd hh:mm:ss");
				}
			}
			/*,
			{name:'errors',index:'errors',sortable:false}*/
		], 
	...
	jQuery(manage_grant_grid_selector).jqGrid('navGrid', manage_grant_pager_selector,
		{ 	// navbar options
			edit: false,
			add: false,
			del: $('#message_delete').val() == 'true' ? true : false,
			delfunc : function () {
				delete_alarm();
			},
